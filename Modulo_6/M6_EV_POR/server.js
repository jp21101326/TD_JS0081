...
app.use(bodyParser.json()); // Para API REST (PUT, DELETE)
app.use(express.static(path.join(__dirname, 'public'))); // Contenido estático (CSS, JS)
...
app.get('/productos', async (req, res, next) => {
  try {
    ...
    //REGISTRO: Error al listar productos (no persistencia)
...
app.post('/productos/agregar', async (req, res, next) => {
  try {
    ...
    //REGISTRO: Error al agregar producto
...
app.post('/productos/editar/:id', async (req, res, next) => {
    try {
        ...
        //REGISTRO: Error al editar producto
        console.error(chalk.red(`[${ENVIRONMENT}] Error en POST /productos/editar/${req.params.id}:`), error.message);
        // En caso de error, siempre devolver JSON para la lógica del fetch
...
app.post('/productos/eliminar/:id', async (req, res, next) => {
    try {
        ...
        //REGISTRO: Error al eliminar producto
...
app.use((err, req, res, next) => {
  // Ya teníamos el registro aquí, lo mantenemos.
